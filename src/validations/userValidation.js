...
const update = async (req, res, next) => {
    const correctCondition = Joi.object({
        ...
        current_password: Joi.string().trim().pattern(PASSWORD_RULE).message(`current_password: ${PASSWORD_RULE_MESSAGE}`),
        new_password: Joi.string().pattern(PASSWORD_RULE).message(`new_password: ${PASSWORD_RULE_MESSAGE}`)
    })
    try {
        // Save if there is an update, otherwise Unknown error
